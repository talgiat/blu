...
function scraperMaker(userId,callback) {
  ...
  return {
    ...
    scrape: function() {
      ...
      parseUrl('http://www.discogs.com/collection?user=' + userId + '&sort=artist,asc&page=1', function(error, $) {
        if (error) {
          ...
          that.callback('Collection for: ' + userId + ' is not public');
        } else {
    ...
    scrapePage: function(pageNum) {
      var that = this;
      parseUrl('http://www.discogs.com/collection?user=' + userId + '&sort=artist,asc&page=' + pageNum, function(error, $, noparsing) {
        if (!encounteredError && !noparsing) {
          if (error) {
...
function parseUrl(url, callback, doit) {
  request( {
    ...
    function (error, response, body) {
      var parseLinks = /(<table[^>]*>)[^$]*(<\/table>)/g;
      var options, window;
      if (!error && response.statusCode === 200) {
        var res = parseLinks.exec(body);
        options = { 
          features: {
